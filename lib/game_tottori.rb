...
def initialize
    ...
    @citizen   = Image.load("image/citizen.png")
		...
		#スプライトクラスのオブジェクトを設定（a?タン、擬似クリa???￣用ポインタ??    @img_next = Sprite.new(275,250, Image.load(File.expand_path("../../image/next.png", __FILE__)))
...
#コンa???￡ニューとENDボタン、擬似ポインタの表示
	self.update(@a)
    @pt.draw
	


    #ボタン処c????次の画面への遷移?? 	if Input.mouseDown?(M_LBUTTON) then
